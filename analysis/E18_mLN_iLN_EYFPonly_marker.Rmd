...
write.table(markerGenes, 
            file= "/Users/immbio/Desktop/Project/Angelina/LNdev_PRC_rev/data/markerGenes_E18_EYFPpos_integrated_intCluster.txt",
            sep="\t",
            quote=F,
            row.names=F,
            col.names=T)
...
## exclude Adipoc and Mesothelial and rerun
```{r}
...
### plot signature 3 all (5selected)
```{r signature 3 all (5selected)}
selGenes <- data.frame(gene=c("Eln","Mfap5","Gsn","Fndc1","Cd34"))
signGenes <- genes %>% dplyr::filter(gene %in% selGenes$gene)
...
#save table
write.table(markerGenes, 
            file= "/Users/immbio/Desktop/Project/Angelina/LNdev_PRC_rev/data/markerGenes_E18_EYFPpos_integrated_fil_label.txt",
            sep="\t",
            quote=F,
            row.names=F,
            col.names=T)
...
## load DEGenesFbcond3
markerGenes <- read.delim("/Users/immbio/Desktop/Project/Angelina/LNdev_PRC_rev/data/markerGenes_E18_EYFPpos_integrated_fil_label.txt", header = TRUE, sep = "\t")
## GSEA 
##adjust table
...
### make heatmap of gnees in signatures
### heatmap funcion
```{r heatmap funcion}
...
```{r negative regulation of stem cell population maintenance}
ego1 <- dplyr::filter(ego@result,ego@result$Description=="negative regulation of stem cell population maintenance")
g1 <- ego1$geneID
Str <-(g1)
StrSub <- strsplit(Str, "/")
df <- as.data.frame(StrSub)
...
### heatmap
```{r heatmap negative regulation of stem cell population maintenance, fig.height=4, fig.width=4}
sig <- markerGenes %>% mutate(geneID = gene) %>% dplyr::filter(cluster == "cluster1") %>%
  mutate(gene=gsub(".*\\.", "",  geneID)) %>% dplyr::filter(gene %in% df$gene)
...
```{r neuronal stem cell population maintenance}
ego1 <- dplyr::filter(ego@result,ego@result$Description=="neuronal stem cell population maintenance")
g1 <- ego1$geneID
Str <-(g1)
StrSub <- strsplit(Str, "/")
df <- as.data.frame(StrSub)
...
### heatmap
```{r heatmap neuronal stem cell population maintenance, fig.height=4, fig.width=4}
sig <- markerGenes %>% mutate(geneID = gene) %>% dplyr::filter(cluster == "cluster1") %>%
  mutate(gene=gsub(".*\\.", "",  geneID)) %>% dplyr::filter(gene %in% df$gene)
...
```{r stem cell differentiation}
ego1 <- dplyr::filter(ego@result,ego@result$Description=="stem cell differentiation")
g1 <- ego1$geneID
Str <-(g1)
StrSub <- strsplit(Str, "/")
df <- as.data.frame(StrSub)
...
### heatmap
```{r heatmap stem cell differentiation, fig.height=6, fig.width=4}
sig <- markerGenes %>% mutate(geneID = gene) %>% dplyr::filter(cluster == "cluster1") %>%
  mutate(gene=gsub(".*\\.", "",  geneID)) %>% dplyr::filter(gene %in% df$gene)
...
### stem cell division
```{r stem cell division}
ego1 <- dplyr::filter(ego@result,ego@result$Description=="stem cell division")
g1 <- ego1$geneID
Str <-(g1)
StrSub <- strsplit(Str, "/")
df <- as.data.frame(StrSub)
...
### heatmap
```{r heatmap stem cell division, fig.height=4, fig.width=4}
sig <- markerGenes %>% mutate(geneID = gene) %>% dplyr::filter(cluster == "cluster1") %>%
  mutate(gene=gsub(".*\\.", "",  geneID)) %>% dplyr::filter(gene %in% df$gene)
